...
import type { EssMe, EssWfhRequest } from "@/types/api";
...
export default function WfhPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      setReason("");
      await refresh();
      setTimeout(() => router.push("/attendance"), 900);
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      <SubHeader title="Work From Home" backHref="/attendance" name={me?.display_name} />
      ...
      <form onSubmit={(e) => void onSubmit(e)} className={`${ui.card} space-y-3 p-4`}>
        <p className="text-sm text-[#434655]">
          Request WFH for manager approval. After approval you can web punch without
          office geofence.
        </p>
      ...
      <section className={`${ui.card} space-y-2 p-4`}>
        <h2 className="text-sm font-bold text-[#0b1c30]">My WFH requests</h2>
        {!rows.length ? (
          <p className="text-sm text-[#434655]">No WFH requests yet.</p>
        ) : (
          <ul className="space-y-2">
            {rows.map((r) => (
              ...
              >
                <span>
                  {r.wfh_date}
                  {r.end_date && r.end_date !== r.wfh_date ? ` → ${r.end_date}` : ""}
                  {" · "}
                  {r.portion.replace("_", " ")}
                </span>
