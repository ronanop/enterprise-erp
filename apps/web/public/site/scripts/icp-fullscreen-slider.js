...
(function (global) {
  ...
  function mount(target, options) {
    ...
    function onScroll() {
      ...
      // Pin still engages at top; scrub starts when section top hits mid-screen
      const pinScrolled = Math.min(Math.max(-rect.top, 0), total);
      pinSlider(pinScrolled, total);
      const progress = Math.min(1, Math.max(0, (mid - rect.top) / total));
  ...
  function ensureSolutionReveal() {
    ...
    const onScroll = () => {
      ...
      // Start when section top hits mid-screen; finish as it leaves mid
      const vh = window.innerHeight || 1;
      const start = vh * 0.5;
      const end = vh * 0.5 - Math.max(rect.height, vh * 0.85);
