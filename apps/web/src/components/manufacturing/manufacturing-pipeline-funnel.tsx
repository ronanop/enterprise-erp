...
import { manufacturingPipelineStages } from "@/config/manufacturing";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface ManufacturingPipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function ManufacturingPipelineFunnel({
  counts,
  loading,
}: ManufacturingPipelineFunnelProps) {
  const values = manufacturingPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Production pipeline</h2>
        <p className="text-[11px] text-muted-foreground">Order → Scrap (FRD-13)</p>
      </div>
            ...
            <li key={stage.key} className="min-w-0">
              ...
              >
                ...
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted">
                  <div
                    className={cn(
                      "h-full rounded-full bg-slate-500 transition-[width] duration-300",
                      index === 1 && "bg-sky-600",
                      index === 2 && "bg-sky-700",
                      index === 3 && "bg-teal-600",
                      index === 4 && "bg-amber-600",
                    )}
                    style={{ width: `${width}%` }}
                    role="presentation"
                  />
