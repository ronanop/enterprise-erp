...
import { analyticsPipelineStages } from "@/config/analytics";
...
export function AnalyticsPipelineFunnel({
  counts,
  loading,
}: AnalyticsPipelineFunnelProps) {
  const values = analyticsPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">BI pipeline</h2>
        <p className="text-[11px] text-muted-foreground">Dataset → Alert (FRD-18)</p>
      </div>
