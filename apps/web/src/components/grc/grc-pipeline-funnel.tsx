...
import { grcPipelineStages } from "@/config/grc";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface GrcPipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function GrcPipelineFunnel({ counts, loading }: GrcPipelineFunnelProps) {
  const values = grcPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">GRC lifecycle</h2>
        <p className="text-[11px] text-muted-foreground">Risk → CAPA (FRD-20)</p>
      </div>
