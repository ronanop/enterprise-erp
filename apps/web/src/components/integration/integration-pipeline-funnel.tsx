...
import { integrationPipelineStages } from "@/config/integration";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface IntegrationPipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function IntegrationPipelineFunnel({
  counts,
  loading,
}: IntegrationPipelineFunnelProps) {
  const values = integrationPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Connectivity funnel</h2>
        <p className="text-[11px] text-muted-foreground">System → Sync (FRD-21)</p>
      </div>
