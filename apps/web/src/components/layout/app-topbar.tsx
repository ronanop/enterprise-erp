...
function workspaceSubtitle(pathname: string, signedIn: boolean): string {
  ...
  if (pathname === "/procurement" || pathname.startsWith("/procurement/")) {
    return "Procurement · secure session";
  }
    ...
    <header className="sticky top-0 z-40 flex h-14 items-center justify-between gap-4 border-b border-border/80 bg-card/80 px-4 backdrop-blur-md supports-backdrop-filter:bg-card/70 sm:px-6">
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {!signedIn ? (
          <Link
            href="/login"
            className="inline-flex h-9 cursor-pointer items-center gap-1.5 rounded-md border border-border bg-primary px-3 text-sm font-medium text-primary-foreground transition-opacity duration-200 hover:opacity-90"
          >
