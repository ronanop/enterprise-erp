...
import { payrollPipelineStages } from "@/config/payroll";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface PayrollPipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function PayrollPipelineFunnel({ counts, loading }: PayrollPipelineFunnelProps) {
  const values = payrollPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Payroll pipeline</h2>
        <p className="text-[11px] text-muted-foreground">Period → Loan (FRD-10)</p>
      </div>
