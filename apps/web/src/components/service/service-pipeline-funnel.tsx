...
import { servicePipelineStages } from "@/config/service";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface ServicePipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function ServicePipelineFunnel({
  counts,
  loading,
}: ServicePipelineFunnelProps) {
  const values = servicePipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        ...
        <p className="text-[11px] text-muted-foreground">SOP ticket workflow</p>
      </div>
