...
import { ecommercePipelineStages } from "@/config/ecommerce";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface EcommercePipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function EcommercePipelineFunnel({
  counts,
  loading,
}: EcommercePipelineFunnelProps) {
  const values = ecommercePipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Commerce funnel</h2>
        <p className="text-[11px] text-muted-foreground">Listing → Return (FRD-22)</p>
      </div>
