...
interface ProjectsPipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function ProjectsPipelineFunnel({
  counts,
  loading,
}: ProjectsPipelineFunnelProps) {
  const values = projectsPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Delivery funnel</h2>
        <p className="text-[11px] text-muted-foreground">Project → Budget (FRD-11)</p>
      </div>
