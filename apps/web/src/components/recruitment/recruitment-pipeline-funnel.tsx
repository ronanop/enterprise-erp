...
import { recruitmentPipelineStages } from "@/config/recruitment";
...
export function RecruitmentPipelineFunnel({
  counts,
  loading,
}: RecruitmentPipelineFunnelProps) {
  const values = recruitmentPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Hiring funnel</h2>
        <p className="text-[11px] text-muted-foreground">Requisition → Onboarding (ERD_13)</p>
      </div>
