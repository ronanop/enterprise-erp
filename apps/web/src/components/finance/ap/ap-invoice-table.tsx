...
type Props = {
  ...
  detailHref?: (row: ApEntry) => string;
  exportTitle?: string;
};

export function ApInvoiceTable(props: Props) {
  ...
  const exportTitle = props.exportTitle ?? "Accounts Payable Invoices";

  const sortable: Partial<Record<ColumnKey, ApSortKey>> = {
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card shadow-sm">
      <div className="sticky top-0 z-10 flex flex-wrap items-center gap-2 border-b border-border/70 bg-card px-3 py-2">
        <Button type="button" variant="outline" size="sm" className="h-8 cursor-pointer gap-1.5" onClick={() => exportApInvoicesCsv(props.rows)}>
          <Download className="size-3.5" /> CSV
        </Button>
        <Button type="button" variant="outline" size="sm" className="h-8 cursor-pointer gap-1.5" onClick={() => exportApInvoicesXlsx(props.rows)}>
          <FileSpreadsheet className="size-3.5" /> XLSX
        </Button>
        <Button type="button" variant="outline" size="sm" className="h-8 cursor-pointer gap-1.5" onClick={() => printApInvoicesTable(exportTitle, props.rows)}>
                ...
                <tr key={row.id} className="border-b border-border/50 transition-colors duration-150 hover:bg-muted/40">
                      ...
                      <Link
                        href={
                          props.detailHref
                            ...
                            : safeEntityHref("/finance/accounts-payable/invoices", row.id)
                        }
                  ...
                  {visible.has("vendor") ? (
                    <td className="px-2 py-1.5">
                      <Link href={safeEntityHref("/finance/accounts-payable/vendors", row.vendor_id)} className="cursor-pointer hover:underline">
