...
import { ArAgingPanel } from "@/components/finance/ar/ar-aging-panel";
...
import { ArInvoiceTable, type ArSortKey } from "@/components/finance/ar/ar-invoice-table";
...
type Tab = "dashboard" | "invoices" | "receipts" | "aging";
...
export function ArHubPage() {
  ...
  const [rows, setRows] = useState<ArEntry[]>([]);
  ...
  const [sortBy, setSortBy] = useState<ArSortKey>("document_date");
  ...
  const loadLookups = useCallback(async () => {
    const res = await resourceService.list("/customers").catch(() => ({ data: [] }));
    const data = res.data;
  ...
  const listDocumentType = tab === "receipts" ? "receipt" : tab === "invoices" ? "invoice" : undefined;
  ...
  }, [page, pageSize, q, status, customerId, currency, fromDate, toDate, dueFrom, dueTo, overdueOnly, listDocumentType, sortBy, sortDir]);

  const loadSummary = useCallback(async () => {
    setSummary(await getArSummary());
  }, []);

  const loadAging = useCallback(async () => {
    setAgingLoading(true);
    try {
      setAgingReport(await getArAging(asOf));
    } catch {
      setAgingReport(null);
    } finally {
  ...
  const load = useCallback(async () => {
    if (!authenticated) {
      setLoading(false);
      setError("Sign in to view accounts receivable.");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      ...
      if (tab === "dashboard" || tab === "invoices" || tab === "receipts") tasks.push(loadList());
      if (tab === "dashboard" || tab === "aging") tasks.push(loadAging());
      await Promise.all(tasks);
    } catch (err) {
      setError(err instanceof ApiClientError ? err.message : "Failed to load accounts receivable");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "/" && !(e.target instanceof HTMLInputElement) && !(e.target instanceof HTMLTextAreaElement)) {
        e.preventDefault();
        document.getElementById("ar-search")?.focus();
      }
    };
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Customer invoices, receipts, aging, and collections."
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            {canPayment ? (
              <>
                <Button type="button" variant="outline" size="sm" className="h-8 cursor-pointer gap-1.5" onClick={() => setReceiptDialogOpen(true)}>
      ...
      <div className="flex flex-wrap gap-1 rounded-lg border border-border/70 bg-muted/30 p-1">
        {([["dashboard", "Dashboard"], ["invoices", "Invoices"], ["receipts", "Receipts"], ["aging", "Aging"]] as const).map(([key, label]) => (
          <button
            key={key}
            type="button"
      ...
      {(tab === "invoices" || tab === "receipts" || tab === "dashboard") ? (
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
          ...
          <FinanceField label="Customer">
            <FinanceSelect value={customerId} onChange={(e) => { setPage(1); setCustomerId(e.target.value); }}>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border/80 bg-card p-3.5 shadow-sm">
            ...
            {loading && rows.length === 0 ? (
              ...
              <p className="mt-3 text-sm text-muted-foreground">No AR invoices yet.</p>
            ) : (
              <ul className="mt-3 divide-y divide-border/60">
                {rows.slice(0, 8).map((r) => (
                  <li key={r.id} className="flex items-center justify-between gap-2 py-2 text-sm">
                    <Link href={`/finance/accounts-receivable/invoices/${r.id}`} className="cursor-pointer hover:underline">
                      <span className="font-mono text-xs text-muted-foreground">{r.document_number}</span>
                      <span className="mx-1.5 text-muted-foreground/40">·</span>
                      {r.customer_name ?? r.customer_code ?? "Customer"}
                    </Link>
      ...
      {tab === "invoices" || tab === "receipts" ? (
        <ArInvoiceTable
          ...
          exportTitle={tab === "receipts" ? "AR Receipts" : "AR Invoices"}
        />
      ) : null}

      {tab === "aging" ? (
        <ArAgingPanel report={agingReport} loading={agingLoading || loading} />
      ) : null}

      <ArInvoiceFormDialog open={invoiceDialogOpen} onClose={() => setInvoiceDialogOpen(false)} onSaved={refreshAll} />
      <ArReceiptDialog open={receiptDialogOpen} onClose={() => setReceiptDialogOpen(false)} onSaved={refreshAll} />
      <ArAllocateDialog open={allocateDialogOpen} onClose={() => setAllocateDialogOpen(false)} onSaved={refreshAll} />
    </div>
  );
}
