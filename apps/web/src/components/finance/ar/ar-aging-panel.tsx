...
import type { ArAgingReport } from "@/services/ar-service";
...
export function ArAgingPanel({ report, loading }: Props) {
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card shadow-sm">
      <div className="sticky top-0 z-10 flex flex-wrap items-center gap-2 border-b border-border/70 bg-card px-3 py-2">
        <h3 className="text-base font-extrabold tracking-tight">AR Aging · As of {asOf}</h3>
        <span className="text-xs text-muted-foreground">Total outstanding: {formatInrPrecise(total)}</span>
        <div className="ml-auto flex flex-wrap gap-2">
          <Button type="button" variant="outline" size="sm" className="h-8 cursor-pointer gap-1.5" onClick={() => exportArAgingCsv(buckets, asOf)}>
            <Download className="size-3.5" /> CSV
          </Button>
          <Button type="button" variant="outline" size="sm" className="h-8 cursor-pointer gap-1.5" onClick={() => exportArAgingXlsx(buckets, asOf)}>
            <FileSpreadsheet className="size-3.5" /> XLSX
          </Button>
          <Button type="button" variant="outline" size="sm" className="h-8 cursor-pointer gap-1.5" onClick={() => printArAgingTable(`AR Aging · ${asOf}`, buckets)}>
              ...
              {report.items.map((row) => (
                <tr key={row.id} className="border-b border-border/40 hover:bg-muted/40">
                  <td className="px-2 py-1.5 font-mono text-xs">
                    <Link href={`/finance/accounts-receivable/invoices/${row.id}`} className="cursor-pointer hover:underline">
                      {row.document_number}
                    </Link>
                  </td>
