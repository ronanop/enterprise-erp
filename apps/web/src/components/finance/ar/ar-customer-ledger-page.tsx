...
import { getCustomerLedger, type ArCustomerLedger } from "@/services/ar-service";
import { formatInrPrecise } from "@/services/finance-service";

export function ArCustomerLedgerPage({ customerId }: { customerId: string }) {
  const [ledger, setLedger] = useState<ArCustomerLedger | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : "Failed to load customer ledger");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={ledger.customer_name ?? ledger.customer_code ?? "Customer ledger"}
        description={`Customer ledger · ${ledger.customer_code ?? customerId.slice(0, 8)}`}
        actions={
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="rounded-xl border border-border/80 bg-card p-3.5 shadow-sm">
          <p className="text-[11px] font-medium tracking-wide text-muted-foreground uppercase">Receipts</p>
          <p className="mt-2 font-mono text-lg tabular-nums">{formatInrPrecise(ledger.receipt_total)}</p>
        </div>
        <div className="rounded-xl border border-border/80 bg-card p-3.5 shadow-sm">
          ...
          <table className="w-full min-w-[960px] border-collapse text-sm">
                ...
                ledger.lines.map((line) => (
                  <tr key={line.id} className="border-b border-border/50 transition-colors duration-150 hover:bg-muted/40">
                    <td className="px-2 py-1.5 font-mono text-xs">
                      <Link href={`/finance/accounts-receivable/invoices/${line.id}`} className="cursor-pointer hover:underline">
            ...
            <tfoot>
              <tr className="sticky bottom-0 border-t border-border/80 bg-muted/50 text-xs font-medium">
                ...
                <td className="px-2 py-2 text-right font-mono tabular-nums">{formatInrPrecise(ledger.receipt_total)}</td>
