...
import { documentsPipelineStages } from "@/config/documents";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface DocumentsPipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function DocumentsPipelineFunnel({
  counts,
  loading,
}: DocumentsPipelineFunnelProps) {
  const values = documentsPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Lifecycle funnel</h2>
        <p className="text-[11px] text-muted-foreground">Document → Archive (FRD-19)</p>
      </div>
