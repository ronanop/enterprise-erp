...
import { helpdeskPipelineStages } from "@/config/helpdesk";
import { cn } from "@/lib/utils";

type StageCount = Record<string, number>;

interface HelpdeskPipelineFunnelProps {
  counts: StageCount;
  loading?: boolean;
}

export function HelpdeskPipelineFunnel({
  counts,
  loading,
}: HelpdeskPipelineFunnelProps) {
  const values = helpdeskPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Support funnel</h2>
        <p className="text-[11px] text-muted-foreground">Ticket → Feedback (FRD-17)</p>
      </div>
