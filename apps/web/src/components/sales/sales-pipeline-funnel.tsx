...
import { salesPipelineStages } from "@/config/sales";
...
export function SalesPipelineFunnel({ counts, loading }: SalesPipelineFunnelProps) {
  const values = salesPipelineStages.map((stage) => ({
    ...stage,
  ...
  return (
    <div className="rounded-xl border border-border/80 bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-base font-extrabold tracking-tight">Order-to-cash pipeline</h2>
        <p className="text-[11px] text-muted-foreground">Quotation → Invoice (FRD-06)</p>
      </div>
