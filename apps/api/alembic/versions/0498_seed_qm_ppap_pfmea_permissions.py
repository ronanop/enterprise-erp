"""Seed PFMEA and PPAP permissions onto existing quality roles."""
...
revision: str = "0498_seed_qm_ppap_pfmea_permissions"
down_revision: str | None = "0497_qm_ppap"
...
NEW_PERM_ROWS = [row for row in QM_PERMISSIONS if row[0] in set(QM_AUTO_PERMISSIONS)]
...
def downgrade() -> None:
    conn = op.get_bind()
    for code in QM_AUTO_PERMISSIONS:
        conn.execute(
            sa.text(
                """
