"""Seed SPC reading permissions onto existing quality roles."""
...
revision: str = "0501_seed_qm_spc_permissions"
down_revision: str | None = "0500_qm_spc_reading"
...
NEW_PERM_ROWS = [row for row in QM_PERMISSIONS if row[0] in set(QM_SPC_PERMISSIONS)]
...
def downgrade() -> None:
    conn = op.get_bind()
    for code in QM_SPC_PERMISSIONS:
        conn.execute(
            sa.text(
                """
