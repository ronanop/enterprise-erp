"""FP-ASSET-006: depreciation update permission, period index, ADEP backfill."""
...
revision: str = "0544_ast_depreciation_governance"
down_revision: str | None = "0543_ast_disposal_governance"
...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    index_name = "ix_ast_asset_depreciation_asset_period_open"
    if not index_exists(bind, TABLE, index_name, schema=SCHEMA):
        op.create_index(
            ...
            postgresql_where=sa.text(
                "is_deleted = false AND status IN ('draft','calculated','posted','failed')"
            ),
        )
    ...
    rows = conn.execute(
        sa.text(
            ...
            FROM asset.ast_asset_depreciation
            WHERE is_deleted = false
              AND document_number LIKE 'ADEP-____-%'
            """
        )
    ).fetchall()
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_ast_asset_depreciation_asset_period_open",
        table_name=TABLE,
        schema=SCHEMA,
    )
