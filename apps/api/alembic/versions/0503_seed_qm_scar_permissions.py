"""Seed SCAR permissions onto existing quality roles."""
...
revision: str = "0503_seed_qm_scar_permissions"
down_revision: str | None = "0502_qm_scar"
...
NEW_PERM_ROWS = [row for row in QM_PERMISSIONS if row[0] in set(QM_SCAR_PERMISSIONS)]
...
def downgrade() -> None:
    conn = op.get_bind()
    for code in QM_SCAR_PERMISSIONS:
        conn.execute(
            sa.text(
                """
