"""FP-ASSET-007: revaluation update permission, open index, AREV backfill."""
...
revision: str = "0472_ast_revaluation_governance"
down_revision: str | None = "0471_ast_depreciation_governance"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

SCHEMA = "asset"
TABLE = "ast_asset_revaluation"
...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    index_name = "ix_ast_asset_revaluation_asset_status_open"
    ...
    rows = conn.execute(
        sa.text(
            ...
            FROM asset.ast_asset_revaluation
            WHERE is_deleted = false
              AND document_number LIKE 'AREV-____-%'
            """
        )
    ).fetchall()
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_ast_asset_revaluation_asset_status_open",
        table_name=TABLE,
        schema=SCHEMA,
    )
