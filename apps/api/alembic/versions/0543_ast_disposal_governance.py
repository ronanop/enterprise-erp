"""FP-ASSET-005: disposal update permission, open disposal index, ADISP backfill."""
...
revision: str = "0543_ast_disposal_governance"
down_revision: str | None = "0542_ast_maintenance_governance"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

SCHEMA = "asset"
TABLE = "ast_asset_disposal"
...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    index_name = "ix_ast_asset_disposal_asset_status_open"
    ...
    rows = conn.execute(
        sa.text(
            ...
            FROM asset.ast_asset_disposal
            WHERE is_deleted = false
              AND document_number LIKE 'ADISP-____-%'
            """
        )
    ).fetchall()
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_ast_asset_disposal_asset_status_open",
        table_name=TABLE,
        schema=SCHEMA,
    )
