"""FP-ASSET-008: audit update permission, planned index, AAUD backfill."""
...
revision: str = "0473_ast_audit_governance"
down_revision: str | None = "0472_ast_revaluation_governance"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

SCHEMA = "asset"
TABLE = "ast_asset_audit"
...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    index_name = "ix_ast_asset_audit_asset_status_open"
    if not index_exists(bind, TABLE, index_name, schema=SCHEMA):
        op.create_index(
            ...
            postgresql_where=sa.text(
                "is_deleted = false AND status IN ('planned','in_progress')"
            ),
        )
    ...
    rows = conn.execute(
        sa.text(
            ...
            FROM asset.ast_asset_audit
            WHERE is_deleted = false
              AND document_number LIKE 'AAUD-____-%'
            """
        )
    ).fetchall()
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_ast_asset_audit_asset_status_open",
        table_name=TABLE,
        schema=SCHEMA,
    )
