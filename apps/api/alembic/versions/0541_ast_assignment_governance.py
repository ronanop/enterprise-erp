"""FP-ASSET-003: assignment update permission, pending index, AASN backfill."""
...
revision: str = "0541_ast_assignment_governance"
down_revision: str | None = "0540_ast_transfer_pending_index"
...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    index_name = "ix_ast_asset_assignment_asset_status_active"
    if not index_exists(bind, TABLE, index_name, schema=SCHEMA):
        op.create_index(
            ...
            postgresql_where=sa.text(
                "is_deleted = false AND status IN ('draft','submitted','approved','active')"
            ),
        )
    ...
    rows = conn.execute(
        sa.text(
            ...
            FROM asset.ast_asset_assignment
            WHERE is_deleted = false
              AND document_number LIKE 'AASN-____-%'
            """
        )
    ).fetchall()
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_ast_asset_assignment_asset_status_active",
        table_name=TABLE,
        schema=SCHEMA,
    )
