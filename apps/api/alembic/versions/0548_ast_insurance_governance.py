"""FP-ASSET-010: insurance action permissions, open index, status constraint expand."""
...
revision: str = "0548_ast_insurance_governance"
down_revision: str | None = "0547_ast_warranty_governance"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

SCHEMA = "asset"
TABLE = "ast_asset_insurance"
...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            f"ALTER TABLE {SCHEMA}.{TABLE} DROP CONSTRAINT IF EXISTS ck_ast_asset_insurance_status"
        )
    )
    op.create_check_constraint(
        "ck_ast_asset_insurance_status",
        TABLE,
        "status IN ('draft','active','renewed','expired','cancelled')",
        schema=SCHEMA,
    )

    index_name = "ix_ast_asset_insurance_asset_status_open"
    if not index_exists(bind, TABLE, index_name, schema=SCHEMA):
        op.create_index(
            ...
            postgresql_where=sa.text(
                "is_deleted = false AND status IN ('active','renewed')"
            ),
        )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_ast_asset_insurance_asset_status_open",
        table_name=TABLE,
        schema=SCHEMA,
    )

    op.drop_constraint("ck_ast_asset_insurance_status", TABLE, schema=SCHEMA, type_="check")
    op.create_check_constraint(
        "ck_ast_asset_insurance_status",
        TABLE,
