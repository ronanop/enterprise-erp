...
from modules.portal.service.engines import DeviceEngine
...
class DeviceService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> PtDevice:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("DeviceService not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        doc = self._numbers.generate(PortalEntityType.DEVICE, cid, PtDevice, "device_number")
        return self._repo.create(ctx, company_id=cid, device_number=doc, **fields)
