...
from modules.document.repository.archive_repository import ArchiveRepository
...
class ArchiveService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> DocArchive:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("ArchiveService not found")
        return row
    ...
    def create(self, ctx: TenantContext, *, branch_id: UUID, company_id: UUID | None = None, **fields):
        ...
        doc = self._numbers.generate(DocEntityType.ARCHIVE, cid, DocArchive, "document_number")
