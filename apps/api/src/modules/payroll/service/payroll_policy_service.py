...
class PayrollPolicyService:
    ...
    def get_active_or_defaults(self, ctx: TenantContext, company_id: UUID | None = None) -> dict:
        ...
        return {
            ...
            "pf_employee_percent": row.pf_employee_percent,
            "pf_employer_percent": row.pf_employer_percent,
            "pf_wage_ceiling": row.pf_wage_ceiling,
            "pf_on_lop": getattr(row, "pf_on_lop", None) or "fixed",
            "sandwich_enabled": bool(getattr(row, "sandwich_enabled", False)),
            "sandwich_off_becomes": getattr(row, "sandwich_off_becomes", None) or "lop",
            "sandwich_triggers": getattr(row, "sandwich_triggers", None) or "unauthorized_absence",
