"""Quality repository base utilities."""
...
class QmScopedRepository(OrgScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    @staticmethod
    def apply_qm_filter(stmt, model, ctx: TenantContext, *, branch_scoped: bool = False):
        stmt = QmScopedRepository.apply_tenant_filter(stmt, model, ctx)
    ...
    def resolve_company_id(ctx: TenantContext, company_id: UUID | None) -> UUID:
        if company_id is not None:
            QmScopedRepository.ensure_company_access(ctx, company_id, module_key=MODULE_KEY)
            return company_id
