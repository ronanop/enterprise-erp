...
class RecScopedRepository(OrgScopedRepository):
    ...
    def resolve_company_id(ctx: TenantContext, company_id: UUID | None) -> UUID | None:
        if company_id is not None:
            RecScopedRepository.ensure_company_access(
                ctx, company_id, module_key=RECRUITMENT_MODULE_KEY
            )
            return company_id
        if has_module_wide_data_access(ctx, RECRUITMENT_MODULE_KEY):
            return None
        allowed = effective_company_ids(ctx, module_key=RECRUITMENT_MODULE_KEY)
