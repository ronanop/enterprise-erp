...
class KpiRepository(AnalyticsScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    def get(self, ctx: TenantContext, row_id: UUID) -> BiKpi | None:
        stmt = select(BiKpi).where(BiKpi.id == row_id, BiKpi.is_deleted.is_(False))
        stmt = self.apply_analytics_filter(stmt, BiKpi, ctx, branch_scoped=False)
        return self.db.scalar(stmt)

    def list_rows(self, ctx: TenantContext, company_id: UUID):
        stmt = select(BiKpi).where(
    ...
    def update(self, ctx: TenantContext, row_id: UUID, **fields) -> BiKpi | None:
