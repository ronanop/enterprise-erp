...
from modules.analytics.service.engines import DataSnapshotEngine
...
class DataSnapshotService:
    def __init__(self, db: Session) -> None:
        ...
        self._engine = DataSnapshotEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> BiDataSnapshot:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("DataSnapshotService not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        doc = self._numbers.generate(AnalyticsEntityType.SNAPSHOT, cid, BiDataSnapshot, "snapshot_number")
        return self._repo.create(ctx, company_id=cid, snapshot_number=doc, **fields)
