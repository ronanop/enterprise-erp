...
from modules.analytics.repository.alert_rule_repository import AlertRuleRepository
...
from modules.analytics.service.engines import AlertRuleEngine
...
class AlertRuleService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> BiAlertRule:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("AlertRuleService not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        doc = self._numbers.generate(AnalyticsEntityType.ALERT, cid, BiAlertRule, "alert_number")
        return self._repo.create(ctx, company_id=cid, alert_number=doc, **fields)
