...
class MktTrendReport(Base, *MktTransactionMixin):
    __tablename__ = "mkt_trend_report"
    __table_args__ = (
        CheckConstraint(
            "status IN ('draft','processing','completed','failed')",
            name="ck_mkt_trend_report_status",
        ),
        {"schema": "marketing"},
    )
    ...
    virality_score: Mapped[float | None] = mapped_column(Numeric(5, 2), nullable=True)
