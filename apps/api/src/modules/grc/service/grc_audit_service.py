...
from modules.grc.repository.audit_repository import AuditRepository
from modules.grc.service.engines import AuditEngine
...
class GrcAuditService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> GrcAudit:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("GrcAuditService not found")
        return row
    ...
    def create(self, ctx: TenantContext, *, branch_id: UUID, company_id: UUID | None = None, **fields):
        ...
        doc = self._numbers.generate(GrcEntityType.AUDIT, cid, GrcAudit, "audit_number")
        return self._repo.create(ctx, company_id=cid, branch_id=branch_id, audit_number=doc, **fields)
