...
from modules.helpdesk.repository.knowledge_article_repository import KnowledgeArticleRepository
from modules.helpdesk.service.document_number_service import DocumentNumberService
from modules.helpdesk.service.engines import KnowledgeArticleEngine
...
class KnowledgeArticleService:
    def __init__(self, db: Session) -> None:
        ...
        self._engine = KnowledgeArticleEngine()
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> HdKnowledgeArticle:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("KnowledgeArticleService not found")
        return row

    def create(self, ctx: TenantContext, company_id: UUID | None = None, **fields):
        cid = self._scope.resolve_company_id(ctx, company_id)
        doc = self._numbers.generate(
            HdEntityType.ARTICLE, cid, HdKnowledgeArticle, "document_number"
        )
