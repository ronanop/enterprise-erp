...
class TicketEscalationService:
    ...
    def get(self, ctx: TenantContext, row_id: UUID) -> HdTicketEscalation:
        row = self._repo.get(ctx, row_id)
        if row is None:
            raise NotFoundException("TicketEscalationService not found")
        return row
    ...
    def create(
        ...
        doc = self._numbers.generate(
            HdEntityType.ESCALATION, cid, HdTicketEscalation, "document_number"
        )
