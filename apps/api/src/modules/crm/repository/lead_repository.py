...
class LeadRepository(CrmScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    def get(self, ctx: TenantContext, row_id: UUID) -> CrmLead | None:
        stmt = select(CrmLead).where(CrmLead.id == row_id, CrmLead.is_deleted.is_(False))
    ...
    def update(self, ctx: TenantContext, row_id: UUID, **fields) -> CrmLead | None:
