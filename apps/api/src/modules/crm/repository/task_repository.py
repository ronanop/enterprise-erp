...
class TaskRepository(CrmScopedRepository):
    def __init__(self, db: Session) -> None:
        super().__init__(db)

    def get(self, ctx: TenantContext, row_id: UUID) -> CrmTask | None:
        stmt = select(CrmTask).where(CrmTask.id == row_id, CrmTask.is_deleted.is_(False))
    ...
    def create(self, ctx: TenantContext, **fields) -> CrmTask:
        row = CrmTask(
            id=uuid4(),
    ...
    def update(self, ctx: TenantContext, row_id: UUID, **fields) -> CrmTask | None:
