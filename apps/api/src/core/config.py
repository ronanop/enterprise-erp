...
from core.infra_resolve import apply_infra_fallback_to_environ
...
def _load_dotenv_files() -> None:
    """Load .env into os.environ (without overriding already-set process env)."""
    for path_str in _ENV_FILES:
        path = Path(path_str)
        if not path.is_file():
            continue
        try:
...
_load_dotenv_files()
_INFRA_RESOLVE_REPORT = apply_infra_fallback_to_environ()
...
class Settings(BaseSettings):
    ...
    infra_fallback_enabled: bool = Field(default=False, alias="INFRA_FALLBACK_ENABLED")
    ...
    celery_result_backend_fallback: str = Field(
        default="", alias="CELERY_RESULT_BACKEND_FALLBACK"
    )
    ...
    minio_endpoint_fallback: str = Field(default="", alias="MINIO_ENDPOINT_FALLBACK")
    ...
    opensearch_url_fallback: str = Field(default="", alias="OPENSEARCH_URL_FALLBACK")
...
infra_resolve_report = _INFRA_RESOLVE_REPORT
