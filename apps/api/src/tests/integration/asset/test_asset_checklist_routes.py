"""HTTP-level integration tests for checklist routes (FP-ASSET-014)."""
...
ITEMS_READY = {
    "items": [{"label": "Inspect housing", "required": True, "result": "pass"}],
}
...
def test_list_requires_read_permission(route_db: Session, route_ids: dict[str, object]) -> None:
    ...
    try:
        ...
        assert "asset.checklist:read" in response.json()["message"]
    finally:
        app.dependency_overrides.clear()
    ...
    try:
        with patch(
            ...
            response = client.post(
                ...
                json={
                    "asset_id": str(uuid4()),
                    "checklist_code": "CHK-FORBIDDEN",
                    "checklist_name": "Forbidden",
                },
            )
        assert response.status_code == 403
        assert "asset.checklist:create" in response.json()["message"]
    finally:
        app.dependency_overrides.clear()


def test_create_success_and_complete(route_db: Session, route_ids: dict[str, object]) -> None:
    ctx = _ctx(route_ids)
    asset = _insert_asset(route_db, route_ids)
    with (
        _authed_client(route_db, ctx) as client,
        patch(
            "modules.asset.service.checklist_service.AuditService.log_entity_change",
            return_value=None,
        ),
    ):
        create_resp = client.post(
            ...
            json={
                ...
                "checklist_code": "CHK-HTTP-001",
                "checklist_name": "HTTP checklist",
                "items_json": ITEMS_READY,
                "status": "completed",
            },
        )
        ...
        row_id = body["id"]
        complete_resp = client.post(f"{BASE}/{row_id}/complete")
...
def test_list_returns_paginated_result(route_db: Session, route_ids: dict[str, object]) -> None:
    ...
    with (
        ...
        client.post(
            ...
            json={
                "asset_id": str(asset.id),
                "checklist_code": "CHK-LIST",
                "checklist_name": "List me",
            },
        )
...
def test_tenant_isolation(route_db: Session, route_ids: dict[str, object]) -> None:
    ...
    with (
        ...
        create_resp = client.post(
            ...
            json={
                "asset_id": str(asset.id),
                "checklist_code": "CHK-TENANT",
                "checklist_name": "Tenant scoped",
            },
        )
...
def test_company_isolation_on_create_employee_returns_not_found(
    ...
    with _authed_client(route_db, other_ctx) as client:
        response = client.post(
            ...
            json={
                "asset_id": str(asset.id),
                "checklist_code": "CHK-COMPANY",
                "checklist_name": "Wrong company",
            },
        )
...
def test_company_mismatch_on_create_returns_422_for_privileged_users(
    ...
    with _authed_client(route_db, ctx) as client:
        response = client.post(
            ...
            json={
                ...
                "asset_id": str(asset.id),
                "checklist_code": "CHK-ADMIN-CO",
                "checklist_name": "Admin mismatch",
            },
        )
    assert response.status_code == 422
...
def _create_draft_checklist(route_db: Session, route_ids: dict[str, object]) -> str:
    ctx = _ctx(route_ids)
    asset = _insert_asset(route_db, route_ids)
    with (
        _authed_client(route_db, ctx) as client,
        ...
        create_resp = client.post(
            ...
            json={
                "asset_id": str(asset.id),
                "checklist_code": f"CHK-RBAC-{uuid4().hex[:6]}",
                "checklist_name": "RBAC draft",
                "items_json": ITEMS_READY,
            },
        )
    assert create_resp.status_code == 200
...
def test_update_requires_update_permission(route_db: Session, route_ids: dict[str, object]) -> None:
    row_id = _create_draft_checklist(route_db, route_ids)
    app = create_app()
    ctx = _ctx(route_ids)
    ...
    try:
        with patch(
            ...
            response = client.patch(
                f"{BASE}/{row_id}",
                json={"checklist_name": "Denied", "version": 1},
            )
        assert response.status_code == 403
        assert "asset.checklist:update" in response.json()["message"]
    finally:
        app.dependency_overrides.clear()


def test_complete_requires_update_permission(route_db: Session, route_ids: dict[str, object]) -> None:
    row_id = _create_draft_checklist(route_db, route_ids)
    app = create_app()
    ctx = _ctx(route_ids)
    ...
    try:
        with patch(
            ...
            response = client.post(f"{BASE}/{row_id}/complete")
        assert response.status_code == 403
        assert "asset.checklist:update" in response.json()["message"]
    finally:
        app.dependency_overrides.clear()
...
def test_cancel_requires_update_permission(route_db: Session, route_ids: dict[str, object]) -> None:
    row_id = _create_draft_checklist(route_db, route_ids)
    app = create_app()
    ctx = _ctx(route_ids)
    ...
    try:
        with patch(
            ...
            response = client.post(f"{BASE}/{row_id}/cancel")
        assert response.status_code == 403
        assert "asset.checklist:update" in response.json()["message"]
    finally:
        app.dependency_overrides.clear()
