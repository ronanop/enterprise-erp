"""HTTP-level integration tests for asset notification routes (FP-ASSET-017)."""
...
BASE = "/api/v1/assets/asset-notifications"
...
def test_create_requires_create_permission(route_db: Session, route_ids: dict[str, object]) -> None:
    ...
    try:
        ...
        assert "asset.notification:create" in response.json()["message"]
    finally:
        app.dependency_overrides.clear()


def test_lifecycle_pending_sent_read_archive(
    ...
    with (
        ...
        patch(
            "modules.asset.service.notification_service.AuditService.log_entity_change",
            return_value=None,
        ),
    ):
        create_resp = client.post(
            ...
            json={
                ...
                "recipient_user_id": str(route_ids["user_id"]),
                "payload_json": {"days": 30},
            },
        )
        ...
        sent = client.post(f"{BASE}/{row_id}/mark-sent")
        assert sent.status_code == 200
        assert sent.json()["data"]["delivery_status"] == "sent"
        assert sent.json()["data"]["sent_at"] is not None

        read = client.post(f"{BASE}/{row_id}/mark-read")
        assert read.status_code == 200
        assert read.json()["data"]["delivery_status"] == "read"

        archive = client.post(f"{BASE}/{row_id}/archive")
        assert archive.status_code == 200
        assert archive.json()["data"]["status"] == "archived"


def test_mark_failed_path(route_db: Session, route_ids: dict[str, object]) -> None:
    ctx = _ctx(route_ids)
    asset = _insert_asset(route_db, route_ids)
    with (
        _authed_client(route_db, ctx) as client,
        ...
        create_resp = client.post(
            ...
            json={
                "asset_id": str(asset.id),
                "notification_type": "audit_due",
                "recipient_user_id": str(route_ids["user_id"]),
            },
        )
        row_id = create_resp.json()["data"]["id"]
        failed = client.post(f"{BASE}/{row_id}/mark-failed")
        assert failed.status_code == 200
        assert failed.json()["data"]["delivery_status"] == "failed"
...
def test_create_rejects_missing_recipient(route_db: Session, route_ids: dict[str, object]) -> None:
    ctx = _ctx(route_ids)
    asset = _insert_asset(route_db, route_ids)
...
def test_tenant_isolation_hides_other_tenant(
    route_db: Session, route_ids: dict[str, object]
) -> None:
    ctx = _ctx(route_ids)
    asset = _insert_asset(route_db, route_ids)
    ...
    with _authed_client(route_db, other_ctx) as client:
        response = client.get(f"{BASE}/{row_id}")
    assert response.status_code == 404


def test_openapi_lists_notification_paths() -> None:
    app = create_app()
    schema = app.openapi()
    paths = schema.get("paths", {})
    assert f"{BASE}" in paths or "/api/v1/assets/asset-notifications" in paths
    assert any("mark-read" in p for p in paths)
    assert any("mark-sent" in p for p in paths)
