"""Asset audit optimistic-locking tests."""
...
def test_repository_rejects_stale_version() -> None:
    ...
    AstAssetAudit.__table__.create(bind=engine, checkfirst=True)
    ...
    row = AstAssetAudit(
        ...
        document_number="AAUD-2026-000001",
        asset_id=uuid4(),
        auditor_employee_id=uuid4(),
        status="planned",
        is_deleted=False,
        version=3,
        created_at=now,
    ...
    with pytest.raises(ConflictException, match="modified by another user"):
        AssetAuditRepository(session).update(ctx, row.id, notes="stale", version=2)
