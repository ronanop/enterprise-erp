"""Asset notification optimistic-locking tests (FP-ASSET-017)."""
...
def test_repository_rejects_stale_version() -> None:
    ...
    AstAssetNotification.__table__.create(bind=engine, checkfirst=True)
    ...
    with pytest.raises(ConflictException, match="modified by another user"):
        AssetNotificationRepository(session).update(
            ctx, row.id, recipient_user_id=uuid4(), version=2
        )
