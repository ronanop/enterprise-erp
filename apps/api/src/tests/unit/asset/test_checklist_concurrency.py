"""Asset checklist optimistic-locking tests (FP-ASSET-014)."""
...
def test_repository_rejects_stale_version() -> None:
    ...
    AstAssetChecklist.__table__.create(bind=engine, checkfirst=True)
    ...
    row = AstAssetChecklist(
        ...
        asset_id=uuid4(),
        checklist_code="CHK-LOCK",
        checklist_name="Locked",
        status="draft",
        is_deleted=False,
        version=3,
        created_at=now,
    ...
    with pytest.raises(ConflictException, match="modified by another user"):
        AssetChecklistRepository(session).update(
            ctx, row.id, checklist_name="stale", version=2
        )
